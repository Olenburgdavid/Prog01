...
public class Rational implements Comparable<Rational>{
    ...
    Rational(BigInteger num , BigInteger den){
        if (den.equals(BigInteger.ZERO)){
            throw new ArithmeticException("Incorrect rational the den cant be equal to zero");
        }
    ...
    Rational add(Rational other){
        return new Rational(num.multiply(other.den).
                add(den.multiply(other.num)),
                den.multiply(other.den));
    }
    Rational substract(Rational other){
